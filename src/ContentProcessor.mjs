 ...
 *
 * @param {(root:boolean, node:{Node}, isFirstPage:boolean)
 *  => void} preambleProcessor
 * @param {(root:boolean, node:{Node}, partNumber:number,
 *  isFirstPage:boolean) => void} partProcessor
 * @param {(config:{object}, root:boolean, node:{Node},
 ...
 *  chapterProcessor
 * @param {Node} rootNode The document root node of the
 *  single html document generated by asciidoctor-chunker.
 *  The node is cloned before processing in this function
...
const processContents = (
  preambleProcessor, partProcessor, chapterProcessor, root, config, basenameMaker) => {
  let chap = 0;
  let part = 0;
  let firstPageProcessed = false;
  let isFirstPage = false;
  root.find('#content').children().each((node, i) => {
