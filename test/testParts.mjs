...
/**
 * Mock for ContentProcessor that processes only parts.
 * @param {*} config 
 * @param {*} root 
 * @param {*} chapterProcessor 
 * @param {*} basenameMaker 
 */
const _processContents = (config, root, partProcessor, basenameMaker) => {
  let chap = 0;
  let part = 0;
  ...
  root.find('#content').children().each((ele, i) => {
    ...
    if (ele.hasClass('sect0')) {
      if (!firstPageProcessed && !isFirstPage) {
        ...
        isFirstPage = false;
      // part extraction
      return partProcessor(config, root, ele, ++part, isFirstPage);
    }
...
test('getPartExtractor()', t => {
  ...
  const partExtractor = getPartExtractor(_printer4test(res), container,
    _documentMaker);
  _processContents(config, orig, partExtractor,
    getFilenameMaker()); // basenameMaker

  // should extract 2 chunks of parts
  t.is(2, res.length);

  // test each extraction
  res.forEach(({ basename, node }) => {
    const cld = node.find('#content').children();
    t.is(2, cld.length); // each #content contains h1.sect0 and div.content
    const h1 = cld.first();
    t.is('h1', h1.tagName);
    t.true(h1.hasClass('sect0'));
    const div = h1.next();
    t.is('div', div.tagName);
    t.true(div.hasClass('partintro'));
    t.true(div.children().first().hasClass('content')); // paragraph of part intro
  });
});
