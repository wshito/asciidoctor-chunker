...
1. Re-writes the relative links (cross references) in order to point to the appropriate chunked files.  The `missing-target` class is added to the anchor when the link target is missing.
